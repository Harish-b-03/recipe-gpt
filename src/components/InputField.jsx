...
const Suggestions = ({search, FilteredIngredients, addIngredient, clearSearch}) => {
  return (
      <div className={`${(search!=="")?'h-[150px] block opacity-100':' h-0 opacity-0 pointer-events-none'} absolute top-full left-[6px] w-[calc(100%-10px)] bg-[rgba(255,255,255,0.05)] rounded-bl-xl rounded-br-xl border border-solid border-[rgba(255,255,255,0.01)] overflow-y-auto transition-all duration-300`}>
        {
          FilteredIngredients.map((item, index)=>{
            return (
              <div key={index} onClick={()=>{addIngredient(item); clearSearch();}} className="min-h-[32px] max-h-fit px-5 py-1 text-gray-400 cursor-pointer hover:bg-[rgba(255,255,255,0.08)] hover:text-white">
                {item.ingredient}
              </div>
            )
          })
...
const InputField = ({loading, clearSearch, onChangeInput, search, FilteredIngredients, addIngredient, handleSubmit}) => {
  return (
    <div>
        ...
        <div className="flex items-center h-14 md:h-10 w-[300px] md:w-[600px]">
            <div className="relative w-full h-full">
                ...
                />
                <Suggestions search={search} FilteredIngredients={FilteredIngredients} addIngredient={addIngredient} clearSearch={clearSearch}/>
            </div>
          <button
            type="submit"
